...
| `tests/<slug>/` (live; lessons 00–09) | one-shot run | correctness check; diffs actual vs expected output |
...
+----------------------+--------------------------+-----------------------+--------------------------+----------------------+
| tests/<slug>/        | does X give the right    | tests/<slug>/main.odin| just verify <slug>       | lessons 00-09 in use |
...
`tests/` is already live: lesson reference solutions (00–09) sit there with `expected.txt` fingerprints, run via `just verify <slug>` / `just verify-all` (`tools/substrate/verify_all.py`). What is still *future* is the claim-backed half — wiring objective wiki claims to these runnable artifacts. The mechanism exists; pointing it at wiki claims is the unbuilt part.
