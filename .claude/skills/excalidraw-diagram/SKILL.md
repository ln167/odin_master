...
### Tree (Hierarchy)
Parent-child branching with connecting lines and free-floating text (no boxes needed). Use for: file systems, org charts, taxonomies. Use `line` elements for the trunk and branches, free-floating text for labels.
...
Instead of full shapes, use small dots (10-20px ellipses) as timeline markers, bullet points, connection nodes, visual anchors.
...
2. **Audit against your original vision** — does the visual structure match the conceptual structure? Does each section use the intended pattern? Does the eye flow as designed? Is hierarchy correct? Are evidence artifacts readable?
3. **Check for visual defects** — clipped/overflowing text, overlaps, arrows crossing elements or landing wrong, ambiguous labels, uneven spacing, cramped vs empty sections, text too small, lopsided composition.
4. **Fix** — edit JSON: widen containers, adjust x/y, add arrow waypoints, reposition labels, resize for balance.
5. **Re-render & re-view.**
6. **Repeat** — until it passes both the vision check and the defect check. Typically 2-4 iterations.

### When to Stop
- Rendered diagram matches the conceptual design
- No text clipped/overlapping/unreadable
...
**Depth & Evidence:** research done; evidence artifacts present; multi-zoom (summary + sections + detail); concrete over abstract; educational value.
**Conceptual:** isomorphism; argues not displays; variety of patterns; no uniform containers.
**Container discipline:** minimal containers; lines as structure; typography hierarchy.
**Structural:** every relationship has an arrow/line; clear flow; hierarchy via size/isolation.
**Technical:** `text` only readable words; `fontFamily: 3`; `roughness: 0`; `opacity: 100`; <30% of text in containers.
**Visual validation:** rendered to PNG and inspected; no overflow; no overlaps; even spacing; arrows land correctly; readable at export size; balanced composition.
