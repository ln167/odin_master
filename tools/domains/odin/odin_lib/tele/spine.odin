...
//
// Canonical machine line (FROZEN CONTRACT, spec §18 verification block; rule §14.9 "the sink is
// sacred"):
//
//     tele cap seq=N ts=T thread=W frame=F name=value type=T proc=P loc=F:L
//
// `seq` is the per-thread monotonic counter (deterministic -> asserted exact by claims); `ts` (QPC
// nanoseconds) and `thread` (OS id) are the first NONDETERMINISTIC fields, so they sit in FIXED
// positions right after seq -> claims wildcard them (`ts=<...> thread=<...>`) while still pinning seq,
// frame, and the payload exactly. `frame` is the spine frame cursor (§18 gap #4): `frame_mark()`
// advances it at one explicit call site (no hidden state, rule §14) and every record stamps the
// current value -- code that never marks records frame 0. The merged stream is ordered by
// (ts, thread, seq); across threads at equal ts the global order is genuinely nondeterministic, so
// claims assert per-thread order + merge invariants, never a global golden interleaving.
//
...
_flush :: proc() {
	...
	slice.sort_by(recs[:], proc(a, b: ^Record) -> bool {
		if a.ts != b.ts {
			return a.ts < b.ts
		}
	...
	for r in recs {
		when _MACHINE {
			_machine_line(fmt.tprintf("tele cap seq=%d ts=%d thread=%d frame=%d %s=%s type=%v proc=%s loc=%s:%d\n",
				r.seq, r.ts, r.thread, r.frame, r.name, string(r.val[:r.vlen]), r.type, r.loc.procedure, _src_path(r.loc.file_path), r.loc.line))
		}
		when _HUMAN {
			...
			// the full coordinate set for agents.
...
// at every dial. Values are already newline-escaped by _det_value; %q quotes/escapes the rest.
@(private, no_instrumentation)
_ndjson_emit :: proc(r: ^Record) {
	if !_NDJSON {
		return
	}
	s := fmt.tprintf(`{{"seq":%d,"ts":%d,"thread":%d,"frame":%d,"name":%q,"value":%q,"type":%q,"proc":%q,"loc":%q}}`+"\n",
		r.seq, r.ts, r.thread, r.frame, r.name, string(r.val[:r.vlen]),
