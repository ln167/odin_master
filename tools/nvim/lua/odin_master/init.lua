...
-- LSP (OLS), format-on-save (odinfmt), DAP (codelldb + odin-dap), the lessons
-- runner, qmd search via <leader>oq, and `:OdinDoctor`.
local M = {}
...
local function register_keymaps(prefix)
    ...
    map("q", function() require("odin_master.search").query() end, "Odin: qmd search the corpus")
    map("r", function() require("odin_master.lessons").run_current() end, "Odin: run current file")
    map("w", function() require("odin_master.watch").toggle() end,       "Odin: toggle run-on-save watch")
