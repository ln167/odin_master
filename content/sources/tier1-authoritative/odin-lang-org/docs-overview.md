...
#### From a ***string literal*** to X [#](#from-a-string-literal-to-x)
...
n.b. This replaces the `core` library container dynamic array [Small\_Array(N, T)](https://github.com/odin-lang/Odin/blob/master/core/container/small_array/small_array.odin).
...
The `bit_set` type models the mathematical notion of a set. A bit\_set’s element type can be either an enumeration or a range:
...
To get the number of elements set, its cardinality, of a bit\_set, use the built-in card procedure:
...
* If *all* of the fields in a `bit_field` are 1-bit in size and are a boolean, please consider using a [`bit_set`](#bit-sets) instead.
...
[raw\_data](https://pkg.odin-lang.org/base/builtin/#raw_data) is a built-in procedure which returns the underlying data of a built-in data type as a [Multi-Pointer](#multi-pointers).
...
| `ODIN_SANITIZER_FLAGS` | A bit\_set indicating the sanitizer flags set using the `-sanitize` command line switch. (`.Address`, `.Memory`, and `.Thread`). |
