...
Unicode codepoint (`i32`). Slicing a string slices _bytes_, not codepoints,
...
Strings are immutable views, but they _do_ point at memory that's owned
...
1. Replace `for r, i in s` with `for i in 0..<len(s) { fmt.println(s[i]) }`.
   You'll print individual _bytes_. Notice how the multi-byte `é` shows up
   as two values that are not by themselves valid runes.
2. Drop `defer delete(greeting)` — the program still prints fine but now
   leaks. We'll see how to _catch_ leaks in lesson 08 (allocators).

## Find more like this

    just substrate-search --bm25 "string vs cstring" --top 5
    just substrate-search --bm25 "strings.concatenate" --top 5
    just substrate-search --bm25 "rune utf8" --top 5

## Next: `content/domains/odin/vault/lessons/05-arrays-fixed/`
